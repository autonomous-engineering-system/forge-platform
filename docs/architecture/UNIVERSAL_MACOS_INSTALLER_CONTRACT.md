...
post-tool snapshot reader, its durable single-assignment store and the
capture/persist/readback producer coordinator are implemented; the privileged
host observation adapter and released
...
request and accepts one canonical context-matched snapshot response. The
authorized privileged helper transport and released wiring remain
unimplemented.
