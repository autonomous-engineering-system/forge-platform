...
requires exact inspection and fresh slot readback, cleans the private staging
set and emits only an identity-bound `READY` receipt. It ships no concrete
privileged adapter or runtime bytes, has no durable recovery bridge, and is not
wired to the released native installer.
