...
only exact canonical requests and canonical context-bound responses. Its named
listener applies a pre-delegate Developer ID Application requirement for the
exact configured installer bundle and Team identities. Concrete host reads, signed
helper registration and released wiring remain outside the implemented
boundary.
...
requests and returns only a canonical context-bound snapshot. Its named listener
admits only the exact signed installer identity through Foundation's
