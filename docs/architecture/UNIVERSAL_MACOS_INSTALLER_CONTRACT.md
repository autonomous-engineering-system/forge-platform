...
the verified tool and gate sets to the request. The fixed-file host-state
reader, publisher, exact durable-readback coordinator and epoch-bracketed exact
source collector are implemented. A concrete descriptor-safe managed-Git
record reader, atomic publisher and exact durable-readback decorator are also
implemented. A descriptor-safe managed-Python record reader is also implemented
with explicit absent-state semantics, atomic publication and exact durable
readback. A descriptor-safe source for one fixed canonical complete five-gate
record, its atomic publisher and a complete-set exact durable-readback decorator
are also implemented. A source-level provider-gate observer now evaluates the
canonical request's exact enabled-provider requirements through an injected
helper-owned inspector and a separate gate router. It passes only exact
verified minimum-version and immutable runtime-identity matches; absence,
authentication requirements and drift remain blocking evidence. The concrete
macOS inspector now derives only exact V3 component-owned executable/home
...
and can restore only the frozen previous runtime. Native code now supplies the
credential-free transport, private staging, read-only archive/Mach-O inspector
and closed runtime-slot mutation coordinator. A native preparation coordinator
now binds those seams to one deterministic session/deployment operation,
requires exact inspection and fresh slot readback, performs retry-safe cleanup
of the private staging set and emits only an identity-bound `READY` receipt.
It ships no concrete privileged adapter or runtime bytes and is not wired to
the released native installer or the platform-neutral executor-journal bridge.
Runtime artifact production and product release-workflow projection remain
separate producer increments. See the
...
saga, explicit deployment-selection in the native wizard, composition/v2
provider targets, native v2 session-plan projection, one-login/multi-target
provider fan-out coordination, the EP system-provisioner adapter, and the Forge
...
mutation route remain unimplemented. A descriptor-safe managed-Python source
adapter now admits one fixed canonical helper-owned record and fails closed for
missing, insecure, malformed, noncanonical or inconsistent evidence; absence
must be explicit. Its paired publisher atomically replaces only secure
canonical prior state and requires exact durable readback. Its live OS observer
and mutation route remain unimplemented. A descriptor-safe gate source now
admits exactly all five gate identities from one fixed canonical helper-owned
record and preserves an explicit failed gate as blocking evidence. Its
...
Forge Platform now contains strict schemas and a tested policy kernel for signed-release selection, canonical GitHub Release identity, structured public signature envelopes and key-ID/threshold gating, sealed V2 trust-configuration and V1 provenance identities, fresh/trusted-clock feed gates, catalog signature/sequence/digest anti-replay, context-bound composition selection, installer capability checks, native Apple Silicon/macOS 26 preflight, managed Git plus exact catalog-approved Python planning, provider gating, system-service declaration checks, and read-only composition diffs. The Python contract recomputes one immutable runtime identity from its exact arm64 artifact, source/build provenance, ABI/tags, policy and minimum macOS fields; components bind build/test evidence to it, product venv identities remain distinct, and the journal freezes target and rollback identities. Its source-level executor persists exact acquisition, inspection, runtime-slot, per-product-venv, activation, final-readback and rollback evidence with crash/reboot resume and no PATH input. Its durable journal bridge reconstructs the exact original `PLANNED` record, validates the executor request and terminal receipt, requires a stable fresh all-`NO_CHANGE` post-tool plan, and is the only durable path into `MANAGED_TOOLS`; generic journal advancement cannot inject that state. The native session parser now independently recomputes that same nested runtime commitment, requires it to equal the signed outer-catalog approval, and retains an exact unique venv binding for every selected Forge/EP component. A native credential-free HTTPS transport acquires only the four assets named by that identity, rejects redirects, final-URL drift, empty or oversized responses and digest drift, and returns identity-bound bytes. A separate native stager writes those roles under fixed names in one `0700` private operation directory, exposes only opaque and descriptor-derived identities, and repeats no-follow filesystem plus exact-digest checks on every readback. A read-only native inspector validates the constrained archive, strict embedded manifest and thin-arm64 Mach-O deployment target without extracting or executing content. A closed runtime-slot coordinator then binds the exact staged file identity and inspection result to a derived immutable slot across an injected privilege seam; it accepts no path or command input and requires staged-byte and independent slot readback before returning `READY`. A native preparation coordinator composes transport/staging, archive inspection, runtime-slot coordination and retry-safe terminal staging cleanup for one deterministic session/deployment operation, and its receipt rebinds the exact runtime, archive, inspection and slot evidence. A strict V1 recovery record preserves that operation's exact four staged identities in bounded installer-owned `0700`/`0600` storage with no-follow reads, exclusive atomic durable creation, identity-matched idempotent save/clear and fail-closed conflict handling. The coordinator acquires one injected host-wide nonblocking lease before state access, recovers any pending record before fresh work, persists each complete staged set before inspection or slot mutation, cleans before clear, and exposes a cleanup-only restart entry point under that same lease. Under the same lease it enumerates unrecorded acquisition directories by descriptor before fresh staging, accepts only the derived operation-name shape and four fixed asset names, and removes them only after revalidating directory and file type, owner, mode, link count and bounded size. The file-backed lock uses a fixed installer-owned private root and single-link private file, rejects symlink, hardlink and permission drift, and fails closed when busy, unavailable or unreleasable. The native core now preserves ordered asset evidence, computes the same execution-request fingerprint as the Python contract, constructs the platform-neutral `COMPLETE` receipt, projects the same typed `TOOLS_VERIFIED` journal evidence, and hands the receipt to an injected idempotent journal bridge before clearing native pending state. A native file-backed store persists one canonical private active `PLANNED` record, independently rebinds the exact request and terminal evidence, and uses durable atomic replacement for an idempotent `MANAGED_TOOLS` transition. The concrete native replanner derives managed Git and Python actions, five fresh non-tool gates, dispatch readiness and a canonical post-tool fingerprint from one strict, context-bound snapshot. Its descriptor-safe private-file reader rejects symlink, hardlink, ownership, mode, shape and context drift. Its paired single-assignment store requires an existing private root, exclusively and atomically writes one context-matched record, synchronizes file and directory descriptors, verifies exact readback, accepts exact retries and rejects conflicts. A producer coordinator validates one complete host observation, persists it and requires identical durable readback before exposing it to the replanner. A strict host-observation adapter derives one closed pathless request and accepts only one canonical context-matched snapshot response. Its authorized privileged post-tool observation and native product-operation XPC transports are source-qualified. The product-operation helper now has a released-authority loader that derives its immutable native release and manifest snapshot solely from the already verified installer context and signed-catalog selections, while historical selections remain installed-only authority. A concrete pinned route resolver now admits only helper-owned typed Forge/EP adapters and pairing executors under exact deployment IDs, rejects cross-route product-instance reuse, and rebinds existing topology before dispatch. A closed helper builder composes those authority and route resolvers around one exact Forge+EP coordinator so admission and dispatch cannot be wired to different registries. Released construction of real adapters and the pairing executor, helper registration, journal seeding and released-installer integration remain unimplemented. No production runtime artifact, live privileged helper registration, native/released executor-journal wiring or released-installer mutation route is approved. The release descriptor, release-operation journal, candidate preparation, packaging, structural evidence verifier, Swift package, and hosted workflow all admit only one thin arm64 installer artifact for macOS 26 or newer; Intel, Rosetta, `x86_64`, fat/universal, and macOS 25 inputs fail closed before platform mutation. The native core now also has a tested C-1 outer-catalog admission kernel, shared strict canonical-JSON/Ed25519 verification primitives, canonical HTTPS validation, bounded JSON parsing, a source-level sealed catalog-trust resource loader, a credential-free exact-locator transport seam, a durable per-scope catalog-anchor store, and C-3a: a read-only coordinator that binds those seams only when independently obtained GitHub HTTPS time attests the exact transport bytes. Its native component-combination parser then accepts only the exact index bytes pinned by that verified outer catalog, preserves separate outer/index identities, applies the same 512 KiB/depth/node limits as the Python qualifier, rechecks both outer/index publication and expiry, and returns only exact-set, explicit-upgrade-route decisions. When a matching accepted index anchor is supplied, it rejects replay and same-sequence/different-byte input; it also rejects supersets and silent fallback from a newer entry whose capability requires a newer installer. `MacOSTrustedInstallerRuntimeBuilder` assembles these read-only catalog, index and manifest paths beside the existing self-update adapters with separate outer/index anchor roots. The unsigned packager can carry a separately validated V1 catalog policy only beside the matched V2/V1 release resources, and the structural archive verifier conditionally binds that policy to the reviewed V2 scope. The native GUI and CLI now select this macOS runtime through a private per-login installer control root. That root contains only self-update and read-only catalog state; product services, venvs, provider credentials and machine-wide mutation locks remain outside it. Source/test bundles with absent sealed resources and malformed state roots still fail closed. The assembled runtime has no production managed-deployment route, provider route, product receipt validator or catalog-acceptance writer. No product mutation or catalog acceptance follows from read-only selection. It also contains a tested native SwiftUI wizard shell, a separate installer release-operation journal with an immutable `PREPARED` candidate precursor and durable sequence reservation, and a source-only release workflow framework. That framework verifies an exact merged `main` candidate, requires its exact version-preparation receipt, requires a reviewed release-identity policy, records required public sequence/provenance inputs, packages an **unsigned** `.app` candidate without manufacturing sealed resources, records the digest of the exact staged archive, and binds the later operation/descriptor handoff to the configured GitHub repository, tag, descriptor asset name, arm64 archive asset name, bundle identifier, Team identifier, CodeDirectory digest and typed notarization receipt. Its signing/notarization and public-GitHub-Release environments deliberately fail closed until a real protected Apple signer, notarization adapter, native trust/provenance loader, descriptor verifier and publisher are configured. The structural handoff verifier enforces the public envelope shape and reviewed identity binding, conditionally validates a bundled catalog policy, but intentionally reports that cryptographic signature verification was not performed; it is never a publication authorization.

The platform-neutral helper now strictly admits canonical native product request
...
no adapter, path, command, environment value or credential. The native source
provides a fixed authorized XPC transport and listener. An immutable helper
resolver selects only exact typed manifest identity/digest pairs from a
preverified authority snapshot and freezes the current installer release.
The released authority loader, product route resolver and helper registration
remain unavailable.
...
dual-readiness evidence. No concrete resolver, released helper process or
released-installer integration is configured. A closed helper service now
composes decoding, helper-owned authority resolution, admission against the
dispatcher's exact registry and dispatch in one call. It emits only a bounded
canonical response and converts every internal boundary failure into one
...
returning it; conflicting or insecure state fails closed. The terminal
coordinator preserves the four ordered asset references and projects this state
into the exact platform-neutral `COMPLETE` receipt, including a request
fingerprint verified against the Python implementation, and exposes the same
typed `TOOLS_VERIFIED` evidence projection as the Python journal bridge. It clears the pending
`READY` receipt only after it freshly re-reads all venvs and the active runtime
under the host-wide lease and an injected idempotent bridge durably accepts that
...
the exact requested tool and gate sets. Its paired publisher atomically replaces
the fixed document only from a private root, rejects insecure or noncanonical
prior state, synchronizes file and directory descriptors and requires exact
durable readback. The source collector reads only the requested identities and
requires matching helper-owned epochs around the complete observation. A
concrete managed-Git record reader now performs stable no-follow readback from
one fixed private helper root. Its atomic publisher and exact durable-readback
decorator are implemented. A managed-Python record reader performs equivalent
...
durable-readback decorator for their fixed canonical complete record are
implemented.
...
ordered venv identities, all-`NO_CHANGE` managed-tool state and explicit
