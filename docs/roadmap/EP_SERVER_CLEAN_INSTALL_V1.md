...
| `FP-EP-CI-5` | Forge Platform | Native exact-runtime transport, archive/Mach-O inspection and privileged bridge using the existing fail-closed executor contract | In progress: transport, private staging with retry-safe exact cleanup and descriptor-safe acquisition-orphan reconciliation, read-only archive/Mach-O inspection, the closed runtime-slot mutation coordinator/protocol, their cleanup-enforcing unprivileged preparation transaction, host-wide nonblocking locking, private atomic pending-record storage, cleanup-only restart integration, idempotent native component-venv/activation coordination with rollback retention and final readback, durable private native `READY`-receipt recovery, cross-implementation `COMPLETE`-receipt and `TOOLS_VERIFIED` evidence projection, an idempotent native journal-handoff coordinator, native fresh-plan parent-journal admission validation, exact required-plus-selected-optional provider binding in the stable plan and canonical V2 helper-request projection, a concrete snapshot-driven native fresh replanner, a strict context-bound post-tool snapshot reader with durable single-assignment store and capture/persist/readback producer coordinator, a closed canonical-response host-observation adapter, a fixed privileged NSXPC client transport, a fail-closed canonical helper service boundary with mutual exact Developer ID peer authentication, a locked single-read helper capture coordinator, a descriptor-safe fixed-file host-state reader, atomic publisher, source/publish/durable-readback coordinator and epoch-bracketed exact source collector, descriptor-safe fixed-record managed-Git and managed-Python readers with atomic publication and exact durable readback, a descriptor-safe complete five-gate record reader with atomic publication and complete-set exact durable readback, a source-level exact provider-gate evaluator, separate gate router, fixed-layout macOS component-provider inspector with immutable pre/post executable verification and fixed noninteractive status probes, bounded credential-free exact V3 provider-archive transport, private descriptor-safe provider-archive staging/readback/discard with strict interrupted-acquisition cleanup, read-only strict `tar.gz`/ZIP provider archive inspection with exact executable-digest and thin-arm64 Mach-O validation, and a host-wide nonblocking cleanup-enforcing provider archive qualification coordinator, a canonical private atomic/idempotent active parent-journal store, an exact `PLANNED` seeding coordinator with durable readback, and the platform-neutral atomic executor-journal bridge are implemented; live managed-Git and managed-Python OS observer/mutation routes, provider archive extraction, installation and credential-home provisioning, service-account secure-store integration, the other four live gate observers, signed service registration and released wiring remain planned |
