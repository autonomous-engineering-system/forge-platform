...
| `FP-EP-CI-5` | Forge Platform | Native exact-runtime transport, archive/Mach-O inspection and privileged bridge using the existing fail-closed executor contract | In progress: transport, private staging with retry-safe exact cleanup, read-only archive/Mach-O inspection, the closed runtime-slot mutation coordinator/protocol and their cleanup-enforcing unprivileged preparation transaction are implemented; a durable recovery record/bridge, concrete privileged adapter/helper and released wiring remain planned |
