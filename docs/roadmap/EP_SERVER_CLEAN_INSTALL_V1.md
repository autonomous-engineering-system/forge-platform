...
| `FP-EP-CI-5` | Forge Platform | Native exact-runtime transport, archive/Mach-O inspection and privileged bridge using the existing fail-closed executor contract | In progress: transport, private staging and read-only archive/Mach-O inspection implemented; privileged bridge and released wiring remain planned |
