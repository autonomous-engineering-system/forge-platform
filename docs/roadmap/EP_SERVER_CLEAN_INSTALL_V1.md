...
| `FP-EP-CI-5` | Forge Platform | Native exact-runtime transport, archive/Mach-O inspection and privileged bridge using the existing fail-closed executor contract | In progress: transport, private staging with retry-safe exact cleanup, read-only archive/Mach-O inspection, the closed runtime-slot mutation coordinator/protocol, their cleanup-enforcing unprivileged preparation transaction, host-wide nonblocking locking, private atomic pending-record storage and cleanup-only restart integration are implemented; acquisition-orphan reconciliation, the executor-journal bridge, concrete privileged adapter/helper and released wiring remain planned |
