...
| `FP-EP-CI-5` | Forge Platform | Native exact-runtime transport, archive/Mach-O inspection and privileged bridge using the existing fail-closed executor contract | In progress: transport, private staging, read-only archive/Mach-O inspection and the closed runtime-slot mutation coordinator/protocol implemented; a concrete privileged adapter/helper and released wiring remain planned |
